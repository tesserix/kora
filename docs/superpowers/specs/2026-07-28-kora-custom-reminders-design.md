...
- **Cap:** `MAX_CUSTOM_REMINDERS = 20`. The "+ Add reminder" affordance is disabled with a one-line note once the cap is reached. Rationale: iOS allows 64 pending local notifications app-wide; meals use ≤4 and each weekday-subset custom reminder uses up to 7, so 20 keeps a comfortable margin.
...
- `applyAllReminders`: `await cancelAllScheduledNotificationsAsync()` once, then `for` each descriptor from `buildSchedule(mealPrefs)` **and** `buildCustomSchedule(customs)` → `scheduleNotificationAsync({ content:{title,body,data}, trigger })`, mapping `{type:"daily"|"weekly"}` to `Notifications.SchedulableTriggerInputTypes.DAILY|WEEKLY`.
