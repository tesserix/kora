...
## RESOLVED — the crash is FIXED (live-verified 2026-07-27 ~16:17)
...
- **Fix (`03e614c`):** `app.json` `ios.entitlements` = `{ "com.apple.developer.healthkit": true, "com.apple.developer.healthkit.access": [] }`, and `bundleIdentifier` changed `com.unidevidp.mobile → com.tesserix.kora`.
- **The clean rebuild SUCCEEDED and the crash is GONE.** `Build Succeeded, 0 errors` → `iOS Bundled … (3810 modules)`. Launched `com.tesserix.kora`, cleared native launch + JS load, signed in `demo@kora.app`, **Home renders and stays stable** (PID alive, no crash reports for the new bundle id, Metro HMR clean/no redbox). The entitlement fix resolved the SIGTRAP.
- **Gotcha for next time:** a stale SIGTRAP crash report at 16:07 was the **OLD** `com.unidevidp.mobile` app — both apps registered the `exp+mobile://` scheme, so the deep-link reopen hit the old one. **Uninstall the old bundle id** (`xcrun simctl uninstall <udid> com.unidevidp.mobile`) to disambiguate.
- **Health state on the sim (NOT a bug):** Steps/Sleep now show a genuine **`0`** (`0 of 10,000` / `0 last night` + rings), **not** "Connect Apple Health". With HealthKit actually linked, the iOS 26.2 sim reports `isHealthDataAvailable()=true` and **auto-grants** authorization → `useHealth` reaches `authorized`, queries return 0 samples → honest 0 (no-fabricated-numbers invariant still holds; `0 ≠ fabricated`). The `connect` state now only shows on `unavailable`/`denied` (a real device where the user declines) — it can't be forced on this sim. Real non-zero steps/sleep need a **physical iPhone**.
