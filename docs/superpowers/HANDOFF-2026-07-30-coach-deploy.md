...
1. **RESOLVED by `kora-fasting-signal`.** `FastingStreakDays` counting today's partial (not-yet-logged) day as fasting was **not** "only ever more protective" as originally claimed here — that claim was wrong. It was a false-positive risk: a brand-new user whose only log was a single partial meal earlier today could be flagged at ED-risk on their very first use (both via `FastingStreakDays` and, independently, via `RecentDeficitPct` and `AvgIntakeKcal`, which had the identical defect). Flagging a fresh user at ED-risk isn't "extra caution" — the mobile UI renders that flag as an eating-disorder support card, and showing it to every first-time user is alarming and desensitising, the opposite of protective. The branch also fixed a second, opposite-direction gap in the same signal: an established, previously-on-target user who went completely silent for a full `recentWindowDays` window or longer used to read as *less* at-risk than a shorter silence, because the in-window anchor the streak counted from aged out — protection was weakest exactly when the gap was longest. Both are fixed: today is now excluded from every RecentDaily-derived signal, unlogged days never count as evidence, and `Context.LoggedBeforeWindow` (backed by `foodlog.Repository.HasLoggedBefore`) lets a whole-window silence still register once the user has logging history from before the window. See `docs/superpowers/specs/2026-07-30-kora-coach-design.md`'s Signals section for the shipped semantics.
2. **RESOLVED by `kora-fasting-signal`.** `RecentDeficitPct` applying today's single kcal target across the 7d window is unchanged (still schema-limited — no historical targets to compare against), but the deficit computation itself is fixed: today is excluded, unlogged days are never scored as a deficit, and a minimum of 2 logged complete days is now required before the signal can be non-zero (previously one abandoned/partial logged day could stand in for the whole window's "average deficit").
