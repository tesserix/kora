...
--
-- Instead, the indexes the read paths actually carry the deleted_at IS NULL
-- predicate on are made partial. Each is dropped and recreated here with
-- identical columns, operator classes and method — only the WHERE clause is
-- new; verified against pg_indexes.indexdef before and after.
DROP INDEX idx_food_items_normalized_name;
CREATE INDEX idx_food_items_normalized_name ON food_items (normalized_name) WHERE deleted_at IS NULL;

DROP INDEX idx_food_items_name_trgm;
CREATE INDEX idx_food_items_name_trgm ON food_items (lower(name)) WHERE deleted_at IS NULL;

DROP INDEX idx_food_items_name;
CREATE INDEX idx_food_items_name ON food_items USING gin (to_tsvector('simple', name)) WHERE deleted_at IS NULL;

DROP INDEX idx_food_items_normalized_name_fts;
CREATE INDEX idx_food_items_normalized_name_fts ON food_items USING gin (to_tsvector('simple', normalized_name)) WHERE deleted_at IS NULL;

DROP INDEX idx_food_items_embedding;
CREATE INDEX idx_food_items_embedding ON food_items USING hnsw (embedding vector_cosine_ops) WHERE deleted_at IS NULL;
...
-- of a silently unattributed row. A guard in bffauth itself would be the
-- better fix but is a different task's file.
--
