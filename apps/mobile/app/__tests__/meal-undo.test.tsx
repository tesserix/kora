...
jest.mock("@/api/hooks", () => ({
  ...
  useCreateLog: () => ({ mutate: mockCreateLogMutate, isPending: false }),
...
test("a correction that taught the index offers Undo and retracts on tap", async () => {
  ...
  // (f2) — this is the single most important assertion in this suite.
  expect(mockEditMutate).toHaveBeenCalledTimes(2);
  const [secondPatch] = mockEditMutate.mock.calls[1];
...
test("a correction that taught nothing undoes without retract_correction", async () => {
  ...
  expect(mockEditMutate).toHaveBeenCalledTimes(2);
  const [secondPatch] = mockEditMutate.mock.calls[1];
  expect(secondPatch).not.toHaveProperty("retract_correction");
  expect(secondPatch).toEqual({
    id: "log1",
    food_item_id: "f1",
    quantity_grams: 200,
...
test("deleting offers Undo which re-creates the log", async () => {
  ...
  expect(mockCreateLogMutate).toHaveBeenCalledTimes(1);
  const [recreateInput] = mockCreateLogMutate.mock.calls[0];
